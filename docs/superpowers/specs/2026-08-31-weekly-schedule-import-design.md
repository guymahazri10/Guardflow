...
Import formats: `.xls`/`.xlsx` (preferred, most accurate) is the only format wired into the upload UI in this phase. `.pdf` text-extraction infrastructure (`parsePdfSchedule`, the `parse-schedule` edge function) is implemented and individually tested, but is **not wired into the UI in this phase** — see Open Questions for why. Image upload/capture stays **deferred to phase 2** (no OCR/Vision capability exists in this project yet; see Open Questions). Only the "אחמ״ש" and "מאבטח" sections of the source file are imported; every other section (בקרה / היעדרויות / חופש / מחלה / מילואים / קורס / לימודים / תגבור) is discarded before it ever reaches storage, a preview, or a warning.
...
1. **Upload** — file picker restricted to `.xls,.xlsx` (PDF and image capture UI deferred, see below), drag-and-drop supported, shows filename/size before submitting.
2. **Processing** — non-blocking: the wizard shows a progress state while `parseExcelSchedule` (client) runs; the rest of the admin screen stays usable in another tab, this isn't a modal blocking the whole app.
...
- **PDF upload is implemented but not wired into the UI in this phase.** `parsePdfSchedule` (client-side grid reconstruction from the PDF text layer) and the `parse-schedule` edge function (server-side PDF parsing) are both real, working, individually tested code — this is not a stub. What's deferred is (a) the UI wiring (the upload wizard no longer accepts `.pdf` — see Client-side integration) and (b) a needed refinement to `parsePdfSchedule`'s grid-clustering: it currently does no X-axis column bucketing, so text items can be assigned to the wrong day column on a real PDF layout. Wiring PDF into the UI before that clustering fix would ship a second, differently-broken import path. Do not delete `parsePdfSchedule.ts`, its tests, or the edge function — this is a UI-surface descope pending a follow-up task, not a rollback of working code.
- **Image / camera capture and scanned-PDF OCR are deferred to phase 2.** No OCR/Vision capability or API key exists in this project today; adding one is a paid external dependency and requires your separate, explicit sign-off (per your standing instruction not to add paid services without approval). The parser architecture (`parseImageSchedule` stub, the `ParseResult.supported` shape) is already built so phase 2 is additive, not a rework.
